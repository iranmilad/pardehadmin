<div>
    @if($type === "single")
        ...
        <div class="image-input image-input-outline image-input-placeholder mb-3" data-kt-image-input="true">
            ...
            <div class="image-input-wrapper w-150px h-150px" @if(!empty($value)) style="background-image: url('{{ asset($value) }}')" @endif></div>
            ...
            <!--end::Tags-->
            <!--begin::حذف-->
            <span class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow tw-absolute -tw-bottom-4 -tw-left-3 remove-image-input" data-bs-toggle="tooltip" title="حذف">
    ...
    @else
        ...
        <div>
            <div class="tw-flex tw-gap-8 md:tw-gap-8 lg:tw-gap-7 tw-flex-wrap multiple_image_input">
                @php $value = !empty($value) ? array_values((array) $value) : []; @endphp
                @foreach($value as $index => $new_value)
                <div class="image-input image-input-outline image-input-placeholder mb-3" data-kt-image-input="true">
                    <!--begin::نمایش existing avatar-->
                    <div class="image-input-wrapper w-100px h-100px" @if(!empty($new_value)) style="background-image: url('{{ asset($new_value) }}')" @endif></div>
                    <!--end::نمایش existing avatar-->
                    <!--begin::Tags-->
                    <button type="button" class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow preview-image-label tw-absolute -tw-top-4 -tw-left-3" data-bs-toggle="tooltip" title="تعویض تصویر">
                        <i class="ki-duotone ki-pencil fs-7">
                    ...
                    <!--end::Tags-->
                    <!--begin::حذف-->
                    <button type="button" data-remove-full="true" data-preview="true" class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow tw-absolute -tw-bottom-4 -tw-left-3 remove-image-input" data-bs-toggle="tooltip" title="حذف">
                        <i class="ki-duotone ki-cross fs-2">
                            <span class="path1"></span>
                            <span class="path2"></span>
                        </i>
